...
//  2026-02-13: Inputs: systems other than X11 are back to starting mouse
//  capture on mouse down (reverts 2025-02-26 change). Only X11 requires waiting
//  for a drag by default (not ideal, but a better default for X11 users). Added
//  ImGui_ImplSDL2_SetMouseCaptureMode() for X11 debugger users. (#3650, #6410,
//  #9235) 2026-01-15: Changed GetClipboardText() handler to return nullptr on
//  error aka clipboard contents is not text. Consistent with other backends.
//  (#9168) 2025-09-24: Skip using the SDL_GetGlobalMouseState() state when one
//  of our window is hovered, as the SDL_MOUSEMOTION data is reliable. Fix macOS
...
//  (#8921) 2025-07-08: Made ImGui_ImplSDL2_GetContentScaleForWindow(),
    ...
    io.SetKeyEventNativeData(
        ...
        (int)event->key.keysym
            .scancode); // To support legacy indexing (<1.87 user code). Legacy
                        // backend uses SDLK_*** as indices to IsKeyXXX()
                        // functions.
    return true;
  }
...
void ImGui_ImplSDL2_SetMouseCaptureMode(ImGui_ImplSDL2_MouseCaptureMode mode) {
  ImGui_ImplSDL2_Data *bd = ImGui_ImplSDL2_GetBackendData();
  if (mode == ImGui_ImplSDL2_MouseCaptureMode_Disabled &&
      bd->MouseCaptureMode != ImGui_ImplSDL2_MouseCaptureMode_Disabled)
...
static void ImGui_ImplSDL2_UpdateMouseData() {
  ...
  // be very inconvenient, so to mitigate the issue on X11 we we wait until
  // mouse has moved to begin capture.
  if (bd->MouseCaptureMode == ImGui_ImplSDL2_MouseCaptureMode_Enabled) {
    SDL_CaptureMouse((bd->MouseButtonsDown != 0) ? SDL_TRUE : SDL_FALSE);
  } else if (bd->MouseCaptureMode ==
             ImGui_ImplSDL2_MouseCaptureMode_EnabledAfterDrag) {
